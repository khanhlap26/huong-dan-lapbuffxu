...
function renderGame(game, activeIndex) {
  ...
  gameContent.append(note, renderMainWebsiteCta());
...
function renderMainWebsiteCta() {
  const section = makeElement("section", "main-website-cta");
  const copy = makeElement("div", "main-website-copy");
  copy.append(makeElement("h2", "", "Truy cập website chính LAPBUFFXU"), makeElement("p", "", "Xem sản phẩm và dịch vụ mới nhất tại LAPBUFFXU."));
  section.append(copy, makeExternalLink(validateUrl(MAIN_WEBSITE_URL), "TRUY CẬP LAPBUFFXU", "button main-website-button"));
  return section;
}
